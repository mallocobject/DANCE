...
ckpt = torch.load(
    "./checkpoints/best_DANCE_emb_snr_-4.pth", map_location="cpu", weights_only=True
)
...
noisy_input = noisy.clone().detach().to(dtype=torch.float32).unsqueeze(0)

with torch.no_grad():
    _ = model(noisy_input)


cache = model.encoder[0].conv[1].vis_cache
