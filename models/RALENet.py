...
class RelativePositionEmbedding(nn.Module):
    def __init__(self, Length, whole_length, num_heads) -> None:
        ...
        relative_coords[:, :, 0] += Length - 1  # shift to start from 0
    ...
    def parameters_normalize(
        ...
        self.relative_position_bias_table.data = (
            torch.randn_like(self.relative_position_bias_table.data) * 0.02
        )
    ...
    def forward(self, R_pos=None):
        ...
        if R_pos is None:
            relative_position_bias = mask_fill(
                relative_position_bias, (attn_length - self.Length) // 2, attn_length
            )
        else:
            relative_position_bias = mask_fill(
                relative_position_bias, R_pos - self.Length // 2, attn_length
            )
...
def mask_fill(mask, init_len, length):
    ...
    return F.pad(
        mask, (init_len, pad_total - init_len, init_len, pad_total - init_len), value=0
    )
...
class RALENet(nn.Module):
    def __init__(
        ...
        self.dtransformer34 = BasicLayer(
            channels[3],
            depth=2,
            num_heads=heads[3],
            local_enhence=high_level_enhence,
